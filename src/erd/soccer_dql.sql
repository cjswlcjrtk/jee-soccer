...
SELECT T.TEAM_NAME, S.STADIUM_NAME, (SELECT TEAM_NAME FROM TEAM WHERE TEAM_ID LIKE SS.AWAYTEAM_ID) , SS.SCHE_DATE
FROM TEAM T JOIN STADIUM S
ON T.STADIUM_ID = S.STADIUM_ID
JOIN SCHEDULE SS
ON S.STADIUM_ID = SS.STADIUM_ID
...
SELECT SS.STADIUM_NAME, S.SCHE_DATE,T.TEAM_NAME 홈팀 ,
(SELECT TEAM_NAME
FROM TEAM
WHERE TEAM_ID LIKE S.AWAYTEAM_ID) 원정팀,
S.HOME_SCORE "홈팀 점수", S.AWAY_SCORE "원정팀 점수"
FROM SCHEDULE S LEFT JOIN TEAM T
ON S.HOMETEAM_ID LIKE T.TEAM_ID
LEFT JOIN STADIUM SS
ON S.STADIUM_ID = SS.STADIUM_ID
WHERE (HOME_SCORE- AWAY_SCORE) >=3
;

SELECT SS.STADIUM_NAME, S.SCHE_DATE,T.TEAM_NAME 홈팀 , TT.TEAM_NAME 원정팀,
S.HOME_SCORE "홈팀 점수", S.AWAY_SCORE "원정팀 점수"
FROM SCHEDULE S LEFT JOIN TEAM T
ON S.HOMETEAM_ID LIKE T.TEAM_ID
LEFT JOIN STADIUM SS
ON S.STADIUM_ID LIKE SS.STADIUM_ID
LEFT JOIN TEAM TT
ON S.AWAYTEAM_ID LIKE TT.TEAM_ID
WHERE (HOME_SCORE- AWAY_SCORE) >=3
...
SELECT P.TEAM_ID , T.TEAM_NAME, ROUND(AVG(P.HEIGHT),3) 평균키
FROM PLAYER P JOIN TEAM T
ON P.TEAM_ID = T.TEAM_ID
...
SELECT T.TEAM_NAME 소속팀명, P.PLAYER_NAME 선수명, P.BACK_NO 백넘버
FROM PLAYER P JOIN TEAM T
ON P.TEAM_ID = T.TEAM_ID
...
FROM PLAYER P JOIN TEAM T
ON P.TEAM_ID = T.TEAM_ID
WHERE HEIGHT < (SELECT AVG(HEIGHT) FROM PLAYER PP WHERE P.TEAM_ID = PP.TEAM_ID GROUP BY TEAM_ID )
;
...
SELECT SS.STADIUM_NAME
FROM SCHEDULE S JOIN STADIUM SS
ON S.STADIUM_ID LIKE SS.STADIUM_ID
...
SELECT 'MF평균',AVG(HEIGHT) FROM PLAYER WHERE POSITION LIKE 'MF'
UNION
SELECT 'DF평균',AVG(HEIGHT) FROM PLAYER WHERE POSITION LIKE 'DF'
UNION
SELECT 'FW평균',AVG(HEIGHT) FROM PLAYER WHERE POSITION LIKE 'FW'
UNION
SELECT 'GK평균', AVG(HEIGHT) FROM PLAYER WHERE POSITION LIKE 'GK'
UNION
SELECT '총평균',AVG(HEIGHT) FROM PLAYER
;
...
SELECT SCHE_DATE,(T.TEAM_NAME||'-'||TT.TEAM_NAME ) 팀들 , S.HOME_SCORE- S.AWAY_SCORE 스코어, ABS(S.HOME_SCORE-S.AWAY_SCORE) 절대값
...
SELECT S.STADIUM_ID, ST.STADIUM_NAME , S.SCHE_DATE
...
SELECT PLAYER_NAME ,T.TEAM_NAME, P.HEIGHT 키,
ROUND((SELECT AVG(HEIGHT)
FROM PLAYER PP
WHERE PP.TEAM_ID LIKE P.TEAM_ID),3) 팀평균
FROM PLAYER P JOIN TEAM T
ON P.TEAM_ID = T.TEAM_ID
ORDER BY ROUND((SELECT AVG(HEIGHT)
FROM PLAYER PP
WHERE PP.TEAM_ID LIKE P.TEAM_ID),3) DESC
;
...
SELECT T.TEAM_ID ,T.TEAM_NAME , ROUND(AVG(P.HEIGHT),3) 팀평균키
FROM PLAYER P JOIN TEAM T
ON P.TEAM_ID = T.TEAM_ID
...
FROM PLAYER P JOIN TEAM T
ON P.TEAM_ID = T.TEAM_ID
WHERE T.TEAM_ID IN ('K07','K09','K08')
...
SELECT COUNT(*)
FROM (SELECT T.TEAM_NAME , P.POSITION, P.BACK_NO, P.PLAYER_NAME,P.HEIGHT
FROM PLAYER P JOIN TEAM T
ON P.TEAM_ID = T.TEAM_ID
WHERE T.TEAM_ID NOT IN ('K07','K09','K08')
